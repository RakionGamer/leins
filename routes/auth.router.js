...
// helper: extrae userId (superAdminId) de un access token expirado (opcional en /refresh)
...
router.post('/login', loginLimiter, async (req, res, next) => {
   try {
      ...
      if (!username || !password) {
         return next(boom.badRequest('Username and password are required', { code: 'BAD_REQUEST' }));
      }
      ...
      return res.status(200).json({
         ...
         data: {
            accessToken,
            refreshToken,
            user: { id: user.id, username: user.username, email: user.email, lastLogin: user.lastLogin, theme: user.theme, name: user.name, last_name: user.last_name }
         }
      });
   } catch (error) {
      return next(error);
...
// --- /logout (pública: requiere body con userId+refreshToken) ---
...
router.get('/sessions', jwtValidate, sessionsGetLimiter, async (req, res, next) => {
   try {
      const userId = req.user.sub; // superAdminId
...
// POST /auth/logout-all  (revocar todas las sesiones activas)
...
// POST /auth/sessions/revoke-others  (revoca todas menos una)
