...
// deriva el estado de pago de una compra a partir del total y el saldo pendiente
...
function ResumenPagina({ items }) {
   let purchased = 0, paid = 0, pending = 0, pendingCount = 0;
   ...
   return (
      <div className="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
         <div className="rounded-2xl border border-border-subtle bg-surface-1 p-4 shadow-sm flex items-center gap-4">
            ...
            <div>
               <div className="text-xs font-semibold text-text-soft uppercase tracking-wide">Total Comprado</div>
               <div className="text-xl font-bold text-heading">{clp(purchased)}</div>
            </div>
         </div>
         <div className="rounded-2xl border border-border-subtle bg-surface-1 p-4 shadow-sm flex items-center gap-4">
            <div className="p-3 bg-emerald-500/10 text-emerald-600 rounded-xl"><BanknotesIcon className="w-6 h-6" /></div>
            <div>
               <div className="text-xs font-semibold text-text-soft uppercase tracking-wide">Total Pagado</div>
               <div className="text-xl font-bold text-heading">{clp(paid)}</div>
            </div>
         </div>
         <div className="rounded-2xl border border-brand/30 bg-brand/5 p-4 shadow-sm flex items-center gap-4 ring-1 ring-brand/10">
            ...
            <div>
               <div className="text-xs font-semibold text-brand uppercase tracking-wide">Por Pagar</div>
         ...
         <div className="rounded-2xl border border-border-subtle bg-surface-1 p-4 shadow-sm flex items-center gap-4">
            ...
            <div>
               <div className="text-xs font-semibold text-text-soft uppercase tracking-wide">Documentos Pendientes</div>
      ...
      <div className="space-y-6">
         <div className="bg-bg-content rounded-3xl p-5 border border-border-subtle shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-5 border-b border-border-subtle">
               <div>
                  <h2 className="text-2xl font-bold text-heading tracking-tight">Cuentas por Pagar</h2>
                  <p className="text-sm text-text-soft mt-1">Facturas de compra, boletas de honorarios y egresos pendientes de pago.</p>
               </div>
               <button
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start bg-surface-1 p-4 rounded-2xl border border-border-subtle/50">
                  <div className="space-y-1.5 md:col-span-3">
                     <label htmlFor="clientFilter" className="block text-xs font-semibold text-text-soft uppercase tracking-wider">Proveedor</label>
                     <input
                        id="clientFilter"
                        disabled={loading}
                        className={ctrl}
                  ...
                  <table className="min-w-full text-sm text-left">
                     <thead className="bg-surface-2 border-b border-border-subtle text-text-soft">
                        <tr>
                           ...
                           <th className="p-4 font-semibold"><div className="flex items-center gap-2"><IdentificationIcon className="w-4 h-4" /> Proveedor</div></th>
                     ...
                     <tbody className="divide-y divide-border-subtle/50">
                        {rows.length === 0 ? (
                           <tr><td colSpan={7} className="p-10 text-center text-text-soft italic">no se encontraron documentos para los filtros aplicados.</td></tr>
                        ) : rows.map((r) => {
