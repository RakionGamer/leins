// src/pages/egress/PurchaseInvoices.jsx
...
export default function PurchaseInvoicesAdapted() {
   ...
   }, [docType, page, pageSize, sort, order, mode, effectiveMonth, fromDate, toDate, debEntityId, sourceFilter]);
   ...
   const handleClearFilters = () => {
      setEntityId(activeEntityId ? String(activeEntityId) : ''); setDocType(''); setSourceFilter(''); setMode('month'); setFilterMonth(todayYYYYMM());
   ...
   useEffect(() => {
      ...
      (async () => {
         try {
            ...
            // seteamos directamente (la db ya nos entrego solo los egresos)
            setRows(mapped);
   ...
   }, [docType, sourceFilter, entityId, mode, sort, order, effectiveMonth, fromDate, toDate, page, pageSize, setSp]);
   ...
   const exportCSV = async () => {
      ...
      try {
         ...
         const header = ['Fecha', 'Folio', 'Tipo', 'Proveedor', 'Exento_CLP', 'Afecto_CLP', 'IVA_CLP', 'Total_CLP', 'Origen'];
         const perPage = 200;
         const allRows = [];
         let currentPage = 1, totalPagesLocal = 1;
         ...
         do {
            ...
            for (const it of items) {
               ...
               const iva = Number(it.vat_amount ?? it.amount_vat ?? 0);
               const total = Number(it.total_amount ?? (exento + afecto + iva));
               const typeName = getSiiDocumentTypeLabel(it.doc_type_code);
               allRows.push([it.issue_date || '', it.folio || '', typeName, it.counterparty_name || it.counterparty_rut || '', exento, afecto, iva, total, it.source || 'SII']);
            }
   ...
   return (
      <div className="space-y-6">
         {/* modal de creacion de compras */}
         <Modal isOpen={isManualModalOpen} onClose={() => setIsManualModalOpen(false)} title="Registrar Compra Manual" maxWidth="max-w-3xl">
            {/* destruimos el componente al cerrar para evitar datos fantasma */}
            {isManualModalOpen && (
               <ManualExpenseForm
                  entityId={entityId}
         ...
         {/* modal de edicion de compras */}
         <Modal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} title="Editar Compra" maxWidth="max-w-3xl">
         ...
         <div className="bg-bg-content rounded-3xl p-5 border border-border-subtle shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 pb-5 border-b border-border-subtle">
               <div>
                  <h2 className="text-2xl font-bold text-heading tracking-tight">Egresos (Compras)</h2>
                  <p className="text-sm text-text-soft mt-1">gestione sus facturas de compra y registros manuales.</p>
               </div>
               <div className="flex flex-wrap items-center gap-3">
                  <SiiSyncButton type="invoices" onSyncSuccess={reloadData} />
                  <button onClick={() => setIsManualModalOpen(true)} className={`${btnCtrl} text-brand border-brand/20 bg-brand/5`} title="Añadir una nueva compra manual">
                     <PlusIcon className="w-5 h-5 stroke-2" /> <span className="hidden sm:inline">Añadir Compra</span>
                  </button>
               </div>
            </div>
            ...
            <div className="space-y-4">
               ...
               <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start bg-surface-1 p-4 rounded-2xl border border-border-subtle/50">
                  <div className={`space-y-1.5 ${mode === 'range' ? 'md:col-span-1' : 'md:col-span-2'}`}>
                  ...
                  <div className={`space-y-1.5 ${mode === 'range' ? 'md:col-span-2' : 'md:col-span-3'}`}>
                     ...
                     <select disabled={loading} className={selectCtrl} value={docType} onChange={(e) => { setDocType(e.target.value); resetToFirstPage(); }}>
                        ...
                        {SII_DOCUMENT_FILTERS.expense.map((option) => (
                  ...
                  <div className={mode === 'range' ? 'md:col-span-5' : 'md:col-span-3'}>
                     {mode === 'month' ? (
                  ...
                  <table className="min-w-full text-sm text-left">
                     <thead className="bg-surface-2 border-b border-border-subtle text-text-soft">
                        <tr>
                           ...
                           <th className="p-4 font-semibold"><div className="flex items-center gap-2"><IdentificationIcon className="w-4 h-4" /> Proveedor (Opc)</div></th>
                           ...
                           <tr key={r.id} className="hover:bg-brand/5 transition-colors group">
                              ...
                              <td className="p-4 text-center">
                                 <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                                    {r.source === 'MANUAL' ? (
                                       <>
                                          <button onClick={() => openEditModal(r)} className="p-1.5 rounded-lg text-text-soft hover:text-brand hover:bg-brand/10 transition outline-none focus:ring-2 focus:ring-brand" title="editar compra">
                                             <PencilSquareIcon className="w-5 h-5" />
                                          </button>
                                          <button onClick={() => handleDelete(r.id)} className="p-1.5 rounded-lg text-text-soft hover:text-danger hover:bg-danger/10 transition outline-none focus:ring-2 focus:ring-danger" title="eliminar compra">
