...
const EditExpenseForm = ({ document, onSuccess, onCancel }) => {
   ...
   useEffect(() => {
      if (document) {
         ...
         // reset formatea y precarga todo el formulario
         reset({
            doc_type_code: document.doc_type_code ? String(document.doc_type_code) : 'null',
            issue_date: safeDate,
   ...
   const onSubmit = async (data) => {
      ...
      try {
         ...
         toast.success('compra actualizada con exito');
         if (onSuccess) onSuccess();

      } catch (err) {
         setGlobalError(err.message || 'ocurrio un error al actualizar la compra');
         scrollToError();
      } finally {
            ...
            <div className="shrink-0 flex items-center gap-4 pb-3 mb-4 border-b border-border-subtle">
               ...
               <div className="flex-1">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase bg-purple-100 text-purple-700 border border-purple-200 dark:bg-purple-900/30 dark:border-purple-800">
                        Registro Manual
                     </span>
                  </div>
                  <p className="text-sm text-text-soft mt-0.5">
                     modifique los datos del documento de compra.
                  </p>
               </div>
            </div>
