...
import { createManualIncome } from 'services/siiDocumentsApi';
...
// ==========================================
// iconos svg locales (se mantienen intactos)
// ==========================================
...
// ==========================================
// esquema de validacion con zod para compras
// ==========================================
const expenseSchema = z.object({
   entity_id: z.string().min(1, 'Debe seleccionar una entidad'),
   doc_type_code: z.string().optional(),
...
const ManualExpenseForm = ({ entityId, onSuccess, onCancel }) => {
   const [globalError, setGlobalError] = useState(null);
   const [isSubmitting, setIsSubmitting] = useState(false);

   // consumimos las entidades usando nuestro super hook de react query
   const { entities, loading: loadingEntities } = useEntities({ limit: 100, assignedAdminOnly: true });
   ...
   // funcion manejadora del submit (ya viene validada por zod)
   const onSubmit = async (data) => {
      ...
      try {
         const payload = {
            ...
            doc_type_code: (!data.doc_type_code || data.doc_type_code === 'null') ? null : Number(data.doc_type_code),
            folio: data.folio || null,
            counterparty_rut: data.counterparty_rut || null,
            counterparty_name: data.counterparty_name || null,
         };

         await createManualIncome(payload);
         toast.success('compra registrada con exito');
         if (onSuccess) onSuccess();

      } catch (err) {
         setGlobalError(err.message || 'ocurrio un error al registrar la compra');
         scrollToError();
      } finally {
      ...
      <div className="relative p-1">
         {isSubmitting && (
            <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-bg-content/60 backdrop-blur-sm rounded-2xl">
               ...
               <p className="mt-4 text-sm font-semibold text-brand">guardando registro...</p>
            </div>
         )}
         ...
         <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col max-h-[75vh]">
            <div className="shrink-0 flex items-center gap-4 pb-3 mb-4 border-b border-border-subtle">
               ...
               <div>
                  ...
                  <p className="text-sm text-text-soft mt-0.5">
                     ingrese compras manuales o facturas no sincronizadas.
                  </p>
               </div>
            </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     ...
                     <div className="space-y-1.5 relative md:col-span-2">
                        <label className="flex items-center gap-2 text-sm font-medium text-text-main">
                           <span className="text-brand opacity-80"><IconBuilding /></span>
                        ...
                        <div className="relative">
                           ...
                           <select
                              disabled={loadingEntities || isSubmitting}
                              className={`${selectCtrl} ${errors.entity_id ? 'border-danger' : ''} ${loadingEntities ? 'italic opacity-70' : ''}`}
                              {...register('entity_id')}
                           >
                              <option value="">{loadingEntities ? 'cargando entidades...' : 'seleccione la entidad'}</option>
                              {entities.map((e) => (<option key={e.id} value={e.id}>{e.name} ({e.rut})</option>))}
                           </select>
                           <span className="absolute right-3.5 top-1/2 -translate-y-1/2 text-text-soft pointer-events-none"><IconChevronDown /></span>
                        </div>
                        {errors.entity_id && <p className="text-danger text-xs mt-1">{errors.entity_id.message}</p>}
                     </div>
                     ...
                     <div className="space-y-1.5 relative md:col-span-2">
                        ...
                        <div className="relative">
                           ...
                           >
                              {SII_DOCUMENT_FILTERS.manualExpense.map((option) => (
            ...
            <div className="shrink-0 flex justify-end gap-3.5 mt-4 pt-4 border-t border-border-subtle">
               <button type="button" onClick={onCancel} className="h-11 px-6 text-sm font-semibold text-text-main bg-surface-1 rounded-2xl border border-border-subtle hover:bg-surface-2 transition-colors focus:outline-none focus:ring-2 focus:ring-border-subtle" disabled={isSubmitting}>
                  Cancelar
               </button>
               <SpinnerButton type="submit" loading={isSubmitting} className="h-11 px-6 text-sm font-semibold text-white bg-brand rounded-2xl hover:bg-brand-hover transition-colors shadow-sm focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2">
                  Guardar Compra
               </SpinnerButton>
            </div>
         </form>
